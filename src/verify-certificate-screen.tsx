...
export function useVerifyHash(): string {
  ...
  const params = useLocalSearchParams<{ hash?: string; h?: string }>();
  const fromPath = pathname.match(/^\/verify\/([a-fA-F0-9]{64})$/)?.[1];
  const raw = String(fromPath ?? params.hash ?? params.h ?? "").trim();
  return raw === "_" ? "" : raw;
}

export function VerifyCertificateView({ hash }: { hash: string }) {
  ...
  useEffect(() => {
    if (!hash) {
      setLoading(false);
      setCert(null);
      setError("Aucun hash de certificat fourni.");
      return;
    }
    let cancelled = false;
    (async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setCert(null);
          setError(e?.detail ?? e?.message ?? "Certificat introuvable");
        }
      } finally {
    ...
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]}>
      <ScrollView contentContainerStyle={styles.scroll}>
        <LinearGradient colors={[NAVY, "#1a3a5c"]} style={[styles.hero, Shadow.cardDark]}>
          ...
          <Text style={styles.heroSub}>Authenticité du certificat d'identité financière</Text>
        </LinearGradient>
          ...
          <View style={[styles.card, styles.invalidCard, Shadow.card]}>
            ...
            {hash ? (
              <>
                ...
                <Text style={styles.hashValue}>{hash.slice(0, 16)}…</Text>
              </>
            ) : null}
          </View>
        ) : null}
