-- Narrow enrollment-notif cleanup: keep contribution "needs info" (/diaspora/proof/…).
...
begin
  ...
  -- Remove only enrollment / pre-access notices (not contribution proof requests).
  delete from public.notifications
  where user_id = v_row.user_id
    and (
      title in ('Dossier Diaspora reçu', 'Inscription Diaspora non validée')
      or coalesce(metadata->>'action_url', '') = '/diaspora'
