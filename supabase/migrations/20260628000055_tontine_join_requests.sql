-- Tontine join requests (public directory → owner approval)
-- Fixes: client-side notification insert failed under RLS; admin dashboard had no tontine requests.

create table if not exists public.tontine_join_requests (
...
create index if not exists tontine_join_req_tontine_idx
  on public.tontine_join_requests (tontine_id, status);
create index if not exists tontine_join_req_requester_idx
  on public.tontine_join_requests (requester_id);

alter table public.tontine_join_requests enable row level security;

drop policy if exists "tontine_join_select" on public.tontine_join_requests;
create policy "tontine_join_select" on public.tontine_join_requests
...
drop policy if exists "tontine_join_insert" on public.tontine_join_requests;
drop policy if exists "tontine_join_update" on public.tontine_join_requests;
